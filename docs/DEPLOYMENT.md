...
首次服务器安装和公网开放已完成：Taskleaf 由 systemd 管理并已开启开机自启；Nginx 分别监听回环和主网卡 IPv4 的 8080 端口，API 仅监听 127.0.0.1:3000。已从外网直连验证页面、健康接口、任务读取及页面静态资源正常，不依赖 SSH 隧道。匿名读写已按用户要求配置，但本次外网检查未修改真实任务。GitHub Actions 自动部署已启用，并于 2026-09-14 通过首次正式发布验收。本文保留通用占位符，不提交真实凭据。
...
已创建 `production` Environment，部署分支策略仅允许 `main`，以上五项 Secrets 已配置；未配置人工审批人。专用账号、SSH 登录及 runner 连通性已验证，经用户确认已设置仓库变量 `DEPLOY_ENABLED=true`。

当前推送 main 会在测试通过后部署；任务完成后的提交确认必须明确提醒这一联动。
...
2026-09-14 上线状态：管理员已完成专用账号初始化，本机及 GitHub runner 的 SSH 登录、发布目录权限、精确 sudo 权限均通过验证。用户已明确同意 commit、push 和部署，部署变量已开启。SSH 只读预检运行 34790249547 成功；首次正式发布运行 34790355403 成功，发布提交为 6227cd7dd583108ac003d8e750b1d5fa278b70f0。已核对 current 指向对应版本，服务 active、公网页面及 JS/CSS 资源返回 HTTP 200，任务接口响应 SHA-256 与发布前一致。本次仅发布代码并重启 Taskleaf，未修改真实任务。
...
工作流支持手动输入 `preflight_only=true`，只读验证 runner SSH、目录及 sudo 权限，不打包、不发布、不重启；即使部署开关关闭也可运行预检，仍仅允许 main 和非 PR 事件。正常手动发布使用 `preflight_only=false`，受部署开关控制。每次 main 推送均会在测试通过后部署，后续提交确认仍须遵守 `agent.md`。停用自动发布可将仓库变量 `DEPLOY_ENABLED` 改为 `false`，这不停止当前网站。